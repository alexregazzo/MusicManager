...
import requests
from flask import Blueprint, redirect, request, session, url_for, render_template, g
...
website = Blueprint("website", __name__, template_folder=settings.TEMPLATES_FOLDER_PATH,
...
def authenticate(f):
    ...
    def auth_wrapper(*args, **kwargs):
        ...
        return redirect(url_for("web.website.page_login"))
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if user.use_password == use_password:
                ...
                return redirect(url_for("web.website.page_overview"))
...
@website.route("/spotify/authentication/acquire")
@authenticate
def spotify_authentication_acquire():
    ...
    try:
        ...
        return redirect(spotify.utils.makeSignInURL(
            ...
            redirect_uri=url_for("web.website.spotify_authentication_response", _external=True),
            state=state,
            scope=spotify.SCOPES,
            show_dialog=settings.DEVELOPMENT))
    except Exception as e:
        logger.exception(e)
    return redirect(url_for("web.website.page_overview", error_message="Um erro ocorreu."))


@website.route("/spotify/authentication/response")
@authenticate
        ...
        if error is None:
            ...
            if req.ok:
                ...
                try:
                    ...
                    return redirect(url_for("web.website.page_overview", success_message="Conta vinculada com sucesso!"))
            else:
